...
function GetWishes(_page) {
    ...
    if(isNaN(_offset)) {var _offset = 0;}   // SI ES NaN PETA
    $.ajax({
        url: '/getWish',
        type: 'POST',
        data: {offset: _offset},
        success: function(res) {
			...
			if(pageRem !=0 ){pageCount = Math.floor(pageCount)+1;}
...
function Edit(elm) {
    $.ajax({
        ...
        success: function(res) {
            ...
            if (data[0]['Private'] == "1") {$('#chkPrivate').attr('checked', 'checked');}      
            if (data[0]['Done'] == "1") {$('#chkDone').attr('checked', 'checked');}
